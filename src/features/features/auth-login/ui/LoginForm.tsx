...
export function LoginForm({ onSuccess, onSwitchRegister, onGuestContinue }: LoginFormProps) {
  const { t } = useI18n()
  const { login } = useAuth()
  const a = t.auth
  ...
  return (
    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
        <button type="button" onClick={() => {}} style={{ background: 'none', border: 'none', fontFamily: THEME.fonts.sans, fontSize: '0.75rem', color: THEME.colors.muted, cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>
          {a.forgotPassword}
        </button>
      </div>
      ...
      <div style={{ textAlign: 'center' }}>
        ...
        <button type="button" onClick={onSwitchRegister} style={{ background: 'none', border: 'none', fontFamily: THEME.fonts.sans, fontSize: '0.8rem', color: THEME.colors.gold, cursor: 'pointer', textDecoration: 'underline', padding: 0 }}>
          {a.switchToRegister}
        </button>
      </div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', margin: '4px 0' }}>
        ...
        <span style={{ fontFamily: THEME.fonts.sans, fontSize: '0.7rem', color: THEME.colors.muted, letterSpacing: '0.05em' }}>
          {a.orContinueAs}
        </span>
        <div style={{ flex: 1, height: '1px', background: '#E8E0D0' }} />
      </div>
